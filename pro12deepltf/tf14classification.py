...
# Binary Cross-Entropy(BCE)는 이진 분류(0 또는 1) 문제에서 모델의 예측 확률값과 실제값 사이의 오차를 계산
...
print('\n3) Model Subclassing 방식(완전 자유로운 형태 - 프로그램 능력이 중요)')
class MyModel(Model):
...
sub_model.compile(loss='binary_crossentropy', \
            optimizer=Adam(learning_rate=0.01), metrics=['accuracy'])
sub_model.fit(x_data, y_data, epochs=20, batch_size=1, verbose=2)
m_eval_sub = sub_model.evaluate(x_data, y_data, verbose=0)
print(m_eval_sub)   # [0.1452949494123459, 1.0]
