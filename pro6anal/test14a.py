...
# 의미 있는 크기를 가지는 지를 검정하는 것을 의미한다.
# 세 집단 이상의 평균 비교에서는 독립인 두 집단의 평균 비교를 반복하여 
# 실시할 경우에 제 1종 오류가 증가하게 되어 문제가 발생한다.
# 이를 해결하기 위해 Fisher가 개발한 분산분석(ANOVA, ANalysis Of Variance)을 
# 이용하게 된다.
...
# 일원분산분산(One way ANOVA)
# 실습) 세 가지 교육 방법을 적용하여 1개월동안 교육받은 교육생 80명을 
# 대상으로 실기시험을 실시.   three_sample.csv'
# 독립변수(범주형) : 한개의 요인: 교육방법, 방법의 종류가 3가지(그룹이 3개)
# 종속변수(연속형) : 실기시험 평균점수

# 귀무: 세 가지 교육 방법에 따른 시험 점수에 차이가 없다.
# 대립: 세 가지 교육 방법에 따른 시험 점수에 차이가 있다.

import pandas as pd
import scipy.stats as stats
from statsmodels.formula.api import ols # 추정 및 검정, 회귀, 시계열 분석 등의
...
print('ANOVA 검정 ---')
#F통계값을 얻기 위해 회귀 분석 결과(linear model)를 사용함
...
# 해석 p: 0.7257 > 0.05 이므로 귀무 채택
# 세 가지 교육방법에 차이가 없다.

# 사후분석(Post Hoc Analysis) 하기
# 세 가지 교육방법에 따른 시험점수에 차이 여부는 알려주지만
# 정확히 어느 그룹의 평균값이 의미가 있는지는 알려주지 않는다. 
# 그룹 간 평균 차이를 구체적으로 알려 주지 않음
# 그러므로 그룹 간의 관계를 보기 위해 추가적인 사후분석이 필요하다.
...
# 각 집단의 표본 수의 차이가 크면 결과에 신뢰가 떨어짐