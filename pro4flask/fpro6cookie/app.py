...
# Cookie는 브라우저에 저장되는 작은 키-값 데이터이고, 서버가 클라이언트와 연결상태를
# 유지하는 것처럼 할 수 있다.
...
<p>
    <a h href="/read_cookie">쿠키 읽기</a>
    <a h href="/delete_cookie">쿠키 삭제</a>
</p>
""" 
...
def set_cookie():
    ...
    # 브라우저는 쿠키를 저장하고, redirect요총에 따라 read_cookie로 다시 요청함

    
@app.get("/read_cookie")
def read_cookie():
    # 브라우저가 요총에 실어 보낸 모든 쿠키 중에서 내 서버가 만든 name 쿠키를 꺼냄
